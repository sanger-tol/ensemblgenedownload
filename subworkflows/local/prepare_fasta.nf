...
workflow PREPARE_FASTA {
    ...
    fasta_gz = ch_compressed_fasta.join(sequence_map).map { _meta, path, ext_meta -> [ext_meta, path] }
    faidx = ch_samtools_faidx.join(sequence_map).map { _meta, path, ext_meta -> [ext_meta, path] }
    gzi = SAMTOOLS_FAIDX.out.gzi.join(sequence_map).map { _meta, path, ext_meta -> [ext_meta, path] }
    sizes = SAMTOOLS_FAIDX.out.sizes.join(sequence_map).map { _meta, path, ext_meta -> [ext_meta, path] }
