...
```bash
nextflow run sanger-tol/ensembldownload --input samplesheet.csv --outdir <OUTDIR> --genome GRCh37 -profile docker
```
...
```bash
nextflow pull sanger-tol/ensembldownload
```
...
First, go to the [sanger-tol/ensembldownload releases page](https://github.com/sanger-tol/ensembldownload/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
